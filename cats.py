...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0:
        return 0.0       
    result = round((counter / len(typed_words) * 100 / 1), 2)
    return result
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    "*** YOUR CODE HERE ***"
    counter = len(typed)
    real_counter = counter / 5
    words_per_second = real_counter / elapsed
    words_per_miniute = words_per_second * 60.0
    result = round(words_per_miniute, 2)
    return result
    # END PROBLEM 4
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        ...
        counter += 1
    while counter > 0:
        for i in range(len(tpp[j]) - 1):
            times[j].append(tpp[j][i + 1] - tpp[j][i])
        counter -= 1
        j += 1   
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        times_for_word = [get_time(times, j, i) for j in player_indices]     
        fastest_player = times_for_word.index(min(times_for_word))
